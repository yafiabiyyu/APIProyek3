    ...
    def extract_data_alternatif(self):
        ...
        for data in range(0,len(data_alternatif)):
            ...
            for id in range(len(data_alternatif[data].subkriteria_id)):
                nama_subkriteria = SubkriteriaDoc.objects(_id=data_alternatif[data].subkriteria_id[id]).first()
                subkriteria_list[data].append(nama_subkriteria.nama_subkriteria)
        return nama_alternatif, nim_alternatif, subkriteria_list
    
    def seleksi(self):
        data_seleksi = []
        list_nama, list_nim,list_subkriteria = self.extract_data_alternatif()
